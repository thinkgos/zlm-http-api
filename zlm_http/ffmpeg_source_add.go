...
type AddFFmpegSourceRequest struct {
	...
	SrcUrl       string  `json:"src_url"`           // M, FFmpeg 拉流地址, 支持任意协议或格式(只要 FFmpeg 支持即可)
	DstUrl       string  `json:"dst_url"`           // M, FFmpeg rtmp 推流地址, 一般都是推给自己, 例如 rtmp://127.0.0.1/live/stream_form_ffmpeg
	TimeoutMs    int     `json:"timeout_ms,string"` // M, FFmpeg 推流成功超时时间
	EnableHls    int     `json:"enable_hls,string"` // M, 是否开启 hls 录制
	EnableMp4    int     `json:"enable_mp4,string"` // M, 是否开启 mp4 录制
	FFmpegCmdKey *string `json:"ffmpeg_cmd_key"`    // O, 配置文件中 FFmpeg 命令参数模板 key(非内容), 置空则采用默认模板:ffmpeg.cmd
}
